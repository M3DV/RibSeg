...
class get_model(nn.Module):
    ...
    def forward(self, xyz, cls_label):
        ...
        cls_label_one_hot = cls_label.view(B,1,1).repeat(1,1,N)#B,16,1
...
class get_loss(nn.Module):
    ...
    def forward(self, pred, target, trans_feat):
        ...
        # tmp=tmp.long().cuda()
        # intersection = tmp.mul(pred)
        # # print('intersection',intersection,intersection.size())
        # i_s = torch.sum(intersection)
        # p_s = torch.sum(pred)
        # t_s = torch.sum(tmp)
        # dice = 1- (2*(i_s)+1)/(p_s+t_s+1)
        # # print('i_s',i_s,i_s.size())
        # # print('p_s',p_s,p_s.size())
        # # print('t_s',t_s,t_s.size())
        # # print('dice',dice,dice.size())
